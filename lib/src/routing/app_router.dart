...
final _tasksBranchNavigatorKey =
    GlobalKey<NavigatorState>(debugLabel: 'tasksBranch');
final _dashboardBranchNavigatorKey =
    GlobalKey<NavigatorState>(debugLabel: 'dashboardBranch');
final _messagesBranchNavigatorKey =
    GlobalKey<NavigatorState>(debugLabel: 'messagesBranch');
final _earningsBranchNavigatorKey =
    GlobalKey<NavigatorState>(debugLabel: 'earningsBranch');
final _paymentsBranchNavigatorKey =
    GlobalKey<NavigatorState>(debugLabel: 'paymentsBranch');
...
final goRouterProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      final isClientProfile = location == '/profile';
      ...
      final isClientNotificationTask =
          RegExp(r'^/client/tasks/[^/]+$').hasMatch(location);
      final isTaskerNotificationRequest =
          RegExp(r'^/tasker/requests/[^/]+$').hasMatch(location);
      final isTaskerNotificationEarning =
          RegExp(r'^/tasker/earnings/[^/]+$').hasMatch(location);
      final isNotificationChat = RegExp(r'^/chat/[^/]+$').hasMatch(location);
      final isTaskerProfile = RegExp(r'^/taskers/\d+$').hasMatch(location);
      final isTaskerReviews =
          RegExp(r'^/taskers/\d+/reviews$').hasMatch(location);
      final isPublicRoute = isAuthRoute || isTaskerProfile || isTaskerReviews;
      final isProtectedTaskRoute = location == '/tasks/create' ||
          RegExp(r'^/tasks/\d+/edit$').hasMatch(location);
      final isTaskDataRoute = isTaskList ||
          isTaskDetail ||
          isNearbyTasks ||
          isProtectedTaskRoute ||
          isClientNotificationTask ||
          isTaskerNotificationRequest;
          ...
          client
              ...
              .then((req) {
                ...
                req.write(jsonEncode({
                  ...
                  'runId': 'pre-fix',
                  'hypothesisId': 'D',
                  'location': 'app_router.dart:71',
                  'msg': '[DEBUG] router redirect evaluated',
                  'data': {
                    'location': location,
                    'authStatus': auth.status.name,
                    'role': auth.user?.role,
                    'splashReady': splash.isReady,
                    'targetLocation': splash.targetLocation
                  },
      ...
      if (auth.status == AuthStatus.authenticated) {
        ...
        if ((isTaskerNotificationRequest || isTaskerNotificationEarning) &&
            !isTasker) {
          return '/dashboard';
        }
        if (isNotificationChat && !isClient && !isTasker) return '/admin';
        if (location.startsWith('/messages') && !isClient && !isTasker) {
          return '/admin';
        }
        if (location.startsWith('/earnings') && !isTasker) {
          return isAdmin ? '/admin' : '/dashboard';
        }
        if (location.startsWith('/payments') && !isClient) {
          return isAdmin ? '/admin' : '/dashboard';
        }
      }
    ...
    routes: [
      GoRoute(
          ...
          builder: (context, state) => DisputeFlowScreen(
              taskId: int.parse(state.uri.queryParameters['taskId']!))),
      GoRoute(
          path: '/disputes',
        ...
        pageBuilder: (context, state) {
          ...
          return CustomTransitionPage(
            ...
            child: MapPickerScreen(
                initialLocation: initial is LatLng ? initial : null),
            transitionsBuilder:
                (context, animation, secondaryAnimation, child) {
      ...
      StatefulShellRoute.indexedStack(
        ...
        branches: [
          ...
          StatefulShellBranch(
              navigatorKey: _messagesBranchNavigatorKey,
              routes: [
                GoRoute(
                    path: '/messages',
          ...
          StatefulShellBranch(
              navigatorKey: _earningsBranchNavigatorKey,
              routes: [
                GoRoute(
                    path: '/earnings',
          ...
          StatefulShellBranch(
              navigatorKey: _paymentsBranchNavigatorKey,
              routes: [
                GoRoute(
                    path: '/payments',
                    name: AppRouteNames.payments,
                    pageBuilder: (context, state) =>
                        const NoTransitionPage(child: ClientPaymentsScreen())),
              ]),
        ],
      ),
    ],
...
class _AppShell extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final clientPaymentsPage =
        auth.user?.isClient == true && navigationShell.currentIndex == 4;
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: DashboardBottomNavigation(
          selectedIndex: auth.user?.isClient == true &&
                  location == '/tasks/create'
              ? 2
              : location.startsWith('/taskers/') && auth.user?.isClient != true
                  ? 4
                  : auth.user?.isClient == true && location == '/profile'
                      ? 4
                      : navigationShell.currentIndex == 2
                          ? (auth.user?.isClient == true ? 3 : 2)
          ...
          earningsLabel: auth.user?.isClient == true
              ? l10n.clientDashboardPayments
              : l10n.dashboardEarnings,
          profileLabel: l10n.dashboardProfile,
          postTaskLabel: auth.user?.isClient == true && !clientPaymentsPage
              ? l10n.dashboardPostTask
              : null,
          onSelected: (index) {
